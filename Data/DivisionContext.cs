...
public class DivisionContext : DbContext
{
    ...
    {
        (_, var division) = await this.GetDivisionInfoAndList(organization, divisionID.ToLower());
        return division;
    }
    ...
    public async Task<List<DivisionInfo>> GetDivisionList(string organization)
    {
        try
        {
            (var infoSet, _) = await this.GetDivisionInfoAndList(organization, string.Empty);
    ...
    bool usesDoubleHeaders)
    {
        ...
        // these are out params to processing method:
        DateTime firstGameDate;
        DateTime lastGameDate;
        List<Standings> standings;
        List<Schedule> schedule;

        (_, var divisionInfo) = await this.GetDivisionInfoAndList(organization, divisionID);
        ...
        if (this.ProcessScheduleFile(scheduleFile, usesDoubleHeaders,
        ...
        bool deleteDivision = false, bool createDivision = false)
    {
        try
        {
            ...
            (var infoSet, var tmpDivisionInfo) = await this.GetDivisionInfoAndList(org, id);
    ...
    public async Task SaveScores(string organization, string divisionID, IList<ScheduleVM> schedules)
    {
        try
        {
            ...
            // division's Updated time needs to change when scores are reported
            (_, var divisionInfo) = await this.GetDivisionInfoAndList(organization, divisionID);
            if ((divisionInfo != null))
            {
                divisionInfo.Updated = this.GetEasternTime();
    ...
    private async Task<(DivisionInfoList?, DivisionInfo?)> GetDivisionInfoAndList(string organization, string divisionID)
    {
        try
        {
    ...
    private bool ProcessScheduleFile(IFormFile scheduleFile, bool usesDoubleHeaders,
